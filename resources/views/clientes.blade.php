                ...
                <div class="flex items-center gap-4 bg-white p-4 border border-[#c4c5da]/10">
                    ...
                    <div class="w-10 h-10 bg-[#0035c5] flex items-center justify-center shrink-0">
                        <span class="text-white font-black text-xs">
                            {{ strtoupper(substr($tc['nombre'] ?? $tc['name'] ?? '?', 0, 1)) }}
                        </span>
                    </div>
                    <div class="flex-1">
                    ...
                    <span class="text-sm font-black text-[#0035c5]">${{ number_format($tc['total_gastado'] ?? 0, 2) }}</span>
                </div>
                @empty
                @for($i = 0; $i < 4; $i++)
                <div class="flex items-center gap-4 bg-white p-4 border border-[#c4c5da]/10">
...
<tr class="fila-cliente border-b border-[#c4c5da]/10 transition-colors"
    ...
    <td class="px-6 py-4 text-center">
        <span class="text-lg font-black">{{ $cliente['num_compras'] }}</span>
    </td>
    ...
    if (cliente.compras && cliente.compras.length > 0) {
        lista.innerHTML = cliente.compras.map(c => `
            <div class="grid grid-cols-12 px-8 py-4 border-b border-[#c4c5da]/10 hover:bg-[#f9f9f9] transition-colors">
                <div class="col-span-3 font-mono text-[10px] text-[#747688]">#${c.id}</div>
